...
function SlideForm({ x = 0, y = 0, bgW, bgH, multiplier = 0, zOffset = 0 }: any) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Refresh and display newest wish
            await fetchWishes();
...
export function CinematicScene() {
  ...
  // Helper to find closest index on mount
  ...
  const triggerSlideTransition = (isDown: boolean) => {
    ...
    if (nextIndex !== activeSlideIndex.current) {
      ...
      // Set a cooldown to prevent rapid multi-slide scrolling
  ...
  useFrame(() => {
    ...
    // Dynamic initialization when scroll container is ready
    if (!initialized.current && maxScroll > 0) {
      activeSlideIndex.current = getClosestSlideIndex(scroll.el.scrollTop, maxScroll, D);
      initialized.current = true;
    }
    
    // Dynamically attach the scroll event listener once scroll.el is ready (Only for 'smooth' mode)
    ...
    if (initialized.current && scroll.el) {
      if (scrollMode === 'smooth' && isUserScrolling.current) {
        // Track the nearest slide index dynamically while scrolling in smooth mode
        activeSlideIndex.current = getClosestSlideIndex(scroll.el.scrollTop, maxScroll, D);
      } else {
        ...
        if (scrollMode === 'strict') {
          // Direct scroll positioning to leverage Drei's ScrollControls built-in damping for perfect, smooth landing!
          scroll.el.scrollTop = targetScroll;
        } else {
          // Smooth snap scrolling for manual momentum tracking, scaled dynamically with transitionSpeed!
